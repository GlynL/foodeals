...
Define the command-line surface for the food-deals catalogue: a `foodeals`
command that lists the catalogue by calling the core's list operation unchanged,
printing every deal to standard output under a count header and failing loudly to
standard error when the core throws. The CLI adds no validation, parsing, or
reshaping of its own; it is a thin surface over the core.
